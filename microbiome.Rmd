...
Comparison of SIN degree between different group 

```{r}
sweetres <- readRDS("data/gyy_sweet.Rdata")
cutoff <- -qnorm(0.05, mean = 0, sd = 1)
outlist <- lapply(sweetres, function(x){x[abs(x) < cutoff] <- 0; x <- sign(x); as(x, "dgCMatrix")})
## degree comparision  
